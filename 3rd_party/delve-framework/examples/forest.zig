...
fn pre_draw() void {
    ...
    const rot_matrix = math.Mat4.direction(billboard_dir, camera.up);
...
fn addGrass(pos: math.Vec3, grass_area: u32, grass_size: f32, density: f32) void {
    ...
    for (0..grass_area) |x| {
        for (0..grass_area) |z| {
            ...
            const hash_value: f32 = xpos + (zpos * 10000);
            const hash_val_int: i32 = @intFromFloat(hash_value);

            var rnd = RndGen.init(@abs(hash_val_int));
            var random = rnd.random();
